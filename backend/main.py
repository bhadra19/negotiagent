...
def negotiate(request: NegotiationRequest) -> dict:
    ...
    audit.log(negotiation_id, "negotiation_completed", {"item": request.item, "quantity": request.quantity, "budget": request.budget, "rounds_completed": result.rounds_completed, "selected_vendor": selected.vendor_id if selected else None, "policy_allowed": decision.allowed if decision else False})
    return {"negotiation_id": negotiation_id, "rounds_completed": result.rounds_completed, "offers": [serialize_offer(offer) for offer in result.offers], "selected_offer": serialize_offer(selected) if selected and decision and decision.allowed else None, "decision": decision.reason if decision else "no offer met the budget"}
